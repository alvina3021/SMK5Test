...
<head>
    ...
    <title>Formulir Motivasi Belajar - SMK5TEST</title>
...
<body class="bg-[#F4F1FF] min-h-screen flex flex-col font-sans">
    ...
    {{-- 1. DATA SOAL LENGKAP (Disalin dari Gambar yang diunggah) --}}
    @php
        $sections = [
            'A. Motivasi Intrinsik' => [
                'Saya merasa senang ketika memahami pelajaran yang sulit. Saya belajar karena ingin menambah pengetahuan dan kemampuan saya.',
                'Saya belajar agar bisa mencapai cita-cita saya.',
                'Saya merasa puas ketika hasil belajar saya meningkat.',
                'Saya berusaha belajar dengan baik walaupun tidak ada yang menyuruh.'
            ],
            'B. Motivasi Ekstrinsik' => [
                'Saya belajar agar mendapat nilai yang tinggi.',
                'Saya belajar karena tidak ingin dimarahi guru atau orang tua.',
                'Saya belajar lebih giat jika ada hadiah atau penghargaan.',
                'Saya bersemangat belajar jika mendapat pujian dari guru.',
                'Saya belajar karena ingin membanggakan orang tua saya.'
            ],
            'C. Ketekunan dan Disiplin Belajar' => [
                'Saya tetap belajar meskipun pelajarannya sulit.',
                'Saya membuat jadwal belajar dan berusaha menaatinya.',
                'Saya selalu mengerjakan tugas sekolah tepat waktu.',
                'Saya tidak mudah menyerah ketika mengalami kegagalan dalam belajar.',
                'Saya lebih memilih belajar daripada bermain jika ada ujian.'
            ],
            'D. Kebutuhan Berprestasi' => [
                'Saya ingin menjadi siswa yang berprestasi di sekolah.',
                'Saya merasa bangga ketika hasil belajar saya lebih baik dari sebelumnya.',
                'Saya ingin menunjukkan kemampuan terbaik saya di setiap pelajaran.',
                'Saya berusaha bersaing secara sehat dengan teman-teman di kelas.',
                'Saya termotivasi untuk terus belajar agar menjadi pribadi sukses di masa depan.'
            ]
        ];
    @endphp
        ...
        <div class="w-full max-w-3xl mx-auto">
            <form action="{{ route('motivasi.store') }}" method="POST" class="space-y-6">
                ...
                <div class="bg-[#0A2A43] rounded-t-xl p-6 text-center shadow-lg">
                    <h1 class="text-2xl font-bold text-white mb-2">Tes Motivasi Belajar</h1>
                ...
                @foreach($sections as $categoryTitle => $questions)
                    ...
                    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 transition hover:shadow-md mb-4">
                        ...
                        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-0">
                            ...
                            <span class="text-xs text-gray-500 font-semibold uppercase tracking-wide w-24 text-center sm:text-left">Tidak Sesuai</span>

                            <div class="flex items-center justify-center gap-6 sm:gap-10 w-full sm:w-auto">
                                {{-- Loop 1 sampai 4 --}}
                                @for ($i = 1; $i <= 4; $i++)
